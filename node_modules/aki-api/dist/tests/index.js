...
const rNumber = [0, 1, 2, 3, 4];
const testGame = async (region, childMode) => {
    const aki = new src_1.Aki({ region });
    await aki.start();
...
(async () => {
    ...
    let selectedRegion = (process.argv.length > 2 ? process.argv.slice(2)[0] : 'all');
    if (!src_1.regions.includes(selectedRegion) && selectedRegion !== 'all') {
        selectedRegion = 'all';
    }
    ...
    if (selectedRegion === 'all') {
        const passed = [];
        const failed = [];
        for (let i = 0; i < src_1.regions.length; i += 1) {
            const region = src_1.regions[i];
            try {
