...
const getServer = async (region) => {
    try {
        ...
        const { data } = await axios_1.default.get(url, { httpsAgent: new https_1.Agent({ rejectUnauthorized: false }) });
...
const request = async (url, checkParamProperty, region, config) => {
    ...
    const { status, data } = await axios_1.default.get(url, Object.assign({ headers, params, httpsAgent: new https_1.Agent({ rejectUnauthorized: false }) }, axiosConfig));
    if (status !== 200 || !data) {
