...
const getTrackSpotify = async (trackId) => {
    return new Promise( async (resolve, reject) => {
        fetch(`https://open.spotify.com/track/${trackId}`).then(async res => {
            ...
            const responseJson = {
                title: trackInfoJson.name,
                id: trackId,
                date: trackInfoJson.albumOfTrack.date.isoString.split('T')[0],
                year: parseInt(trackInfoJson.albumOfTrack.date.year),
                views: trackInfoJson.playcount,
                duration: trackInfoJson.duration.totalMilliseconds,
                artists: artistsArray,
                thumb: trackInfoJson.albumOfTrack.coverArt.sources[2].url,
    ...
    return new Promise(async (resolve, reject) => {
        fetch(`https://open.spotify.com/search/${query}`).then(async res => {
            ...
            for(let obj of searchInfoJson.topResults.itemsV2) {
                ...
                response.bestResultsArray.push(bestJson)
            }
            ...
            for(let obj of searchInfoJson.albums.items) {
                ...
                response.albunsArray.push(albumsJson)
            }
            ...
            for(let obj of searchInfoJson.playlists.items) {
                ...
                response.playlistsArray.push(playlistJson)
            }
...
const prepareZip = async (titleAlbum, trackArray, client = require('@adiwajshing/baileys').default(), from, isNotify) => {
    return new Promise(async (resolve, reject) => { 
        const zip = new AdmZip()
        var isTryAgain = 0
        for(let i = 0; i < trackArray.length; ++i) {
            try {
                var obj = trackArray[i]
                const artistsArray = []
                for(obj2 of obj.artists) artistsArray.push(obj2.name)
                artistsArray.push('Downloaded by Alastor bot')

                var requestAudio = (await fetch(obj.audioUrl, {
                    headers: {
                        'User-Agent': 'PostmanRuntime/7.32.2',
                        'Accept': '*/*',
                        'Accept-Encoding': 'gzip, deflate, br',
                ...
                var buffAudio = await requestAudio.buffer()
                const writer = new ID3Writer(buffAudio)
                await writer.setFrame('TIT2', obj.title)
                await writer.setFrame('TALB', titleAlbum)
                await writer.setFrame('TPE1', artistsArray)
                await writer.setFrame('TPE2', artistsArray[0])
                await writer.setFrame('WOAS', 'https://open.spotify.com/track/' + obj.id)
                await writer.setFrame('WOAR', 'https://open.spotify.com/track/' + obj.id)
                await writer.setFrame('TCON', ['Soundtrack'])
                await writer.setFrame('WPAY', 'https://open.spotify.com')
                await writer.setFrame('WORS', 'https://open.spotify.com')
                await writer.setFrame('WPUB', 'https://open.spotify.com')
                await writer.setFrame('WOAF', 'https://open.spotify.com/track/' + obj.id)
                if(obj.year) await writer.setFrame('TYER', obj.year)
                await writer.setFrame('TRCK', `${i+1}/${trackArray.length}`)
                await writer.setFrame('USLT', { lyrics: obj.lyricsStr, language: 'eng', description: 'Letra da música'});
                await writer.setFrame('APIC', { type: 3, data: await getBuffer(obj.thumb), description: 'Spotify'})
                await writer.addTag();
                const bufferWriter = Buffer.from(writer.arrayBuffer)
                
                if(Buffer.byteLength(bufferWriter) < 1048576 && isTryAgain == 0) {
                    --i;
                    isTryAgain += 1
                    console.log('Try again')
                    continue
                } else if(Buffer.byteLength(bufferWriter) < 1048576 && isTryAgain >= 3) {
                    client.sendMessage(from, {
                        text: `_*❌ Falha ao baixar música ${i+1}/${trackArray.length}! Continuando... ❌*_`
                    })
                    isTryAgain = 0
                    continue
                } else isTryAgain = 0
                await zip.addFile(obj.title+'.mp3', bufferWriter, obj.title)
                if(isNotify) client.sendMessage(from, {
                    text: `_*✅ Música ${i+1}/${trackArray.length} baixada com sucesso! ✅*_`
                })
            } catch (e) {
                console.log(e);
                client.sendMessage(from, {
        ...
        resolve({
            zip: zipBuffer,
            size: Buffer.byteLength(zipBuffer)
        })
        return
    })
}
